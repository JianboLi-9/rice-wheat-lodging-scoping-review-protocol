# Protocol v2.1a CURRENT — Full Protocol plus prospective Amendments v2.1 and v2.1a
...
> **Formal database search: NOT STARTED at amendment creation**

## Amendment control

This file carries forward the complete substantive content of `Protocol_v2.0_FINAL.md` and applies `Protocol_Amendment_v2.1_Registration_Timing_20260819.md` plus `Protocol_Amendment_v2.1a_Public_Availability_20260820.md`. The amendments change only administrative registration/public-availability timing; they do not change review design, research questions, Population, Concept, Context, Eligibility, language/year/publication-type rules, database set, WOS-A-001, Stage 2, screening, extraction, robustness, Study Family rules or bibliometric pending decisions. v2.0 remains preserved as the immutable historical pre-conduct protocol.
...
**Version:** v2.1a CURRENT (v2.0 substantive content frozen)  
**Revision date:** 2026-08-20  
**Stage:** Stage 2 completed; prospective pre-formal-search administrative amendments  
**Status:** CURRENT CONTENT FROZEN; PUBLIC PROTOCOL AVAILABILITY REQUIRED; FORMAL DATABASE SEARCH NOT STARTED
...
Bibliometric corpus尚未冻结。语料库冻结后，可能进行：

- Performance Analysis：annual scientific production、source productivity、authors、institutions、countries、citations。
- Science Mapping：co-authorship、citation、co-citation、bibliographic coupling、keyword co-occurrence、thematic evolution。

每种分析要记录为什么做、对应RQ、数据、软件、计数方法、参数时点、报告和解释。软件方法基础包括bibliometrix/Biblioshiny（Aria & Cuccurullo 2017及官方bibliometrix文档）与VOSviewer（van Eck & Waltman及官方文件文档）。所有分析只在final corpus frozen后执行；不为图形外观反向改变检索范围，不在本阶段运行Biblioshiny或VOSviewer。
...
**Stage 2 search-strategy development complete. v2.0 substantive Protocol content FINAL FROZEN; v2.1 and v2.1a prospective administrative amendments recorded before formal search. Formal database search NOT started. OSF public action remains DEFERRED, not cancelled; public protocol availability remains a pre-conduct requirement.** Scopus is excluded from the prespecified formal set for access/resource reasons; CNKI remains an approved future Chinese-language evidence-identification source and was not formally searched. HG-01 IEEE terminology verification is closed; HG-02/HG-03 remain open administrative actions that do not replace the public-protocol availability requirement.

Protocol v2.1a CURRENT is the operative pre-search view. Under AMEND-2.1-REG-TIMING plus AMEND-2.1A-PUBLIC-AVAILABILITY, formal Stage 3 evidence identification may precede the human OSF public action only after a legitimate public version of the current protocol is available, the updated preflight passes and user authorization is recorded; the actual OSF public registration date must be recorded later and both amendments must be reported transparently. Development searches used to construct and validate the protocol search strategy are not formal review searches and are not PRISMA identification counts.

本完整协议草案完成后立即停止。不得重新执行WoS/Scopus/IEEE/CNKI/Google Scholar检索、重新导出、筛选、Pilot Screening、PRISMA、Biblioshiny、VOSviewer、Results或Discussion。
